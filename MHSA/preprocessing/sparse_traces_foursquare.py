...
class DataFoursquare(object):
    def __init__(self, city, trace_min=10, global_visit=10, hour_gap=72, min_gap=10, session_min=2, session_max=10,
                 sessions_min=2, train_split=0.8, embedding_len=50):
        tmp_path = "./data/"

        self.TWITTER_PATH = tmp_path + 'dataset_TSMC2014_{}.txt'.format(city)
        self.SAVE_PATH = tmp_path

        self.save_name = 'foursquare_{}'.format(city.lower())
        
        print(f"Input file set to: {self.TWITTER_PATH}")
        print(f"Output file will be named: {self.save_name}.pk")
...
def parse_args():
    ...
    parser.add_argument('--city', type=str, required=True, help="the city to process, e.g., NYC or TKY")
...
if __name__ == '__main__':
    ...
    data_generator = DataFoursquare(city=args.city,
                                    trace_min=args.trace_min, global_visit=args.global_visit,
