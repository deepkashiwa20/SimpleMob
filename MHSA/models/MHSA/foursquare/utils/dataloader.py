...
class sp_loc_dataset(torch.utils.data.Dataset):
    ...
    def _splitDataset(self, totalData):
        """Split dataset into train, vali and test using the existing 'split' column."""
        
        if "split" not in totalData.columns:
            raise ValueError("'split' column not found in the dataset. Please add it first.")
        
        train_data = totalData.loc[totalData["split"] == "train"].copy()
        vali_data = totalData.loc[totalData["split"] == "valid"].copy()
        test_data = totalData.loc[totalData["split"] == "test"].copy()

        train_data.drop(columns={"split"}, inplace=True)
        vali_data.drop(columns={"split"}, inplace=True)
    ...
    def ___getValidSequenceUser(self, df):                          
        ...
        for traj_id in tqdm(set(df["trajectory_id"].to_list())):
            traj_df = df[df["trajectory_id"] == traj_id]

            for index, (idx, row) in enumerate(traj_df.iterrows()):
