...
def run(args):
    ...
    if args.pretrain == 1:
        model.load_state_dict(torch.load(f"../pretrain/{args.data_name}/" + args.model_mode + "/res.m"))
    ...
    for epoch in range(parameters.epoch):
        ...
        avg_loss, avg_acc, users_acc, acc5, acc10, ndcg5, ndcg10, mrr = run_simple(data_test, test_idx, 'test', lr, parameters.clip, model,
                                                  optimizer, criterion, parameters.model_mode)
        print('==>Test Acc:{:.4f} Acc@5: {:.4f} Acc@10: {:.4f} Ndcg@5: {:.4f} Ndcg@10: {:.4f} MRR: {:.4f} Loss:{:.4f}'.format(avg_acc, acc5, acc10, ndcg5, ndcg10, mrr, avg_loss))
...
def load_pretrained_model(config):
    res = json.load(open(f"../pretrain/{config.data_name}/" + config.model_mode + "/res.txt"))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='../results/nyc', help="path to save results")
    parser.add_argument('--model_mode', type=str, default='attn_local_long', choices=['simple', 'simple_long', 'attn_avg_long_user', 'attn_local_long'])
