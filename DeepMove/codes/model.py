...
class TrajPreLocalAttnLong(nn.Module):
    ...
    def forward(self, loc, tim, target_len):
        ...
        seq_len = x.size(0)
        hist_len = seq_len - target_len  # 可能为 0（第一条轨迹）

        # --- Encoder ---
        if hist_len > 0:
            enc_in = x[:-target_len]  # 历史
            if self.rnn_type in ('GRU', 'RNN'):
                hidden_history, h1 = self.rnn_encoder(enc_in, h1)
            else:  # LSTM
                hidden_history, (h1, c1) = self.rnn_encoder(enc_in, (h1, c1))
            hidden_history = hidden_history.squeeze(1)  # [hist_len, hidden_size]
        else:
            # 第一条轨迹：没有历史，跳过 encoder
            hidden_history = None  # 标记一下，后面 attention 分支用
            # h1/c1 已经是零初始化了，不需要动

        # --- Decoder（当前/目标部分）---
        dec_in = x[-target_len:]  # 若 hist_len==0，这里就是整个 x
        if self.rnn_type in ('GRU', 'RNN'):
            hidden_state, h2 = self.rnn_decoder(dec_in, h2)
        else:  # LSTM
            hidden_state, (h2, c2) = self.rnn_decoder(dec_in, (h2, c2))
        hidden_state = hidden_state.squeeze(1)  # [target_len, hidden_size]

        # --- Attention / Context ---
        if hidden_history is not None and hidden_history.size(0) > 0:
            attn_weights = self.attn(hidden_state, hidden_history).unsqueeze(0)  # [1, tgt_len, hist_len]
            context = attn_weights.bmm(hidden_history.unsqueeze(0)).squeeze(0)   # [tgt_len, hidden_size]
        else:
            # 没有历史：不给注意力，context 置零（或用 nn.Parameter 学一个）
            context = torch.zeros_like(hidden_state)
