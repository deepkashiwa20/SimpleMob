...
from tools import entropy_spatial


class DataSet(object): # dataset_name: nyc, tky, ca


    def __init__(self, dataset_name='nyc', trace_min=10, global_visit=10, hour_gap=72, min_gap=10, session_min=2, session_max=10,
                 sessions_min=2, train_split=0.8, embedding_len=50):
        tmp_path = "data/"
        self.dataset_name = dataset_name
        self.TWITTER_PATH = tmp_path + self.dataset_name + '/tweet_clean_all.txt'
        self.VENUES_PATH = tmp_path + self.dataset_name + '/venues_all.txt'
        self.SAVE_PATH = tmp_path
        self.save_name = f'{self.dataset_name}_cut_one_day'
    ...
    def load_trajectory_from_tweets(self):
        with open(self.TWITTER_PATH,encoding='UTF-8') as fid:
            for i, line in enumerate(fid):
                ...
                if uid not in self.data:        # 制作 data[uid], 每个uid包括[pid, time, cat_id]
                    self.data[uid] = [[pid, tim, cat_id]]
                else:
                    self.data[uid].append([pid, tim, cat_id])
                if pid not in self.venues:        # 统计 poi 次数
                    self.venues[pid] = 1
                else:
                    self.venues[pid] += 1
                if cat_id not in self.venues_cat: # 统计 poi类别 次数
    ...
    def filter_users_by_length(self):
        uid_3 = [x for x in self.data if len(self.data[x]) > self.trace_len_min]                    # uid集合， 排除小于10条轨迹的用户          ---- filter 1
        xixi = [(x, len(self.data[x])) for x in uid_3]      # [uid, 轨迹数]集合
        pick3 = sorted([(x, len(self.data[x])) for x in uid_3], key=lambda x: x[1], reverse=True)   # [uid, 轨迹数]集合， 按轨迹数降序排列
        pid_3 = [x for x in self.venues if self.venues[x] > self.location_global_visit_min]         # pid集合， 排除小于10次访问数的poi         ---- filter 2
        pid_pic3 = sorted([(x, self.venues[x]) for x in pid_3], key=lambda x: x[1], reverse=True)   # [pid, 访问数]集合， 按访问数降序排列
        pid_3 = dict(pid_pic3) # 转为字典，方便直接用pid查询

        session_len_list = []
        for u in pick3:  # u = (uid, 轨迹数)
            uid = u[0]  # 取出uid
            info = self.data[uid] # info = [[pid, time, cat_id], ...] 即取得这一个用户的所有轨迹
            xixi = Counter([x[0] for x in info]) # 返回字典， 统计该用户访问过的poi及访问次数
            topk = Counter([x[0] for x in info]).most_common() # 返回字典{ poi : 访问次数 }， 统计该用户访问过的poi及访问次数， 按访问次数降序排列
            topk1 = [x[0] for x in topk if x[1] > 1] # 访问次数大于1的poi集合                                                                  ---- filter 3
            sessions = {}
    ...
    def save_variables(self):
        ...
        pickle.dump(dataset, open(self.SAVE_PATH + self.save_name + '.pkl', 'wb'))


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset_name', default='nyc', help="dataset name: nyc, kumamoto11, kumamoto7, tky, ca")
...
if __name__ == '__main__':
    args = parse_args()
    data_generator = DataSet(dataset_name=args.dataset_name, trace_min=args.trace_min, global_visit=args.global_visit,
