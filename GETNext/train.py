...
def train(args):
    ...
    # %% ====================== Define Dataset ======================
    class TrajectoryDatasetTrain(Dataset):                    # 按照trajectory划分数据集，83228条样本，11022个轨迹，每个轨迹除去第一条，剩下的作为label，还剩72206条样本
        def __init__(self, train_df):
            ...
            for traj_id in tqdm(set(train_df['trajectory_id'].tolist())):
                ...
                poi_ids = traj_df['POI_id'].to_list() # 这里poi id可能有重复，但是每一个都会遍历到，所以不会过滤
                ...
                if len(input_seq) < args.short_traj_thres: # default 2
                    continue
    ...
    class TrajectoryDatasetVal(Dataset):
        def __init__(self, df): # 18299
            ...
            for traj_id in tqdm(set(df['trajectory_id'].tolist())):
                ...
                if user_id not in user_id2idx_dict.keys():     #这部分过滤了0条
                    continue
                ...
                for each in poi_ids:
                    if each in poi_id2idx_dict.keys():
                        ...
                        # Ignore poi if not in training set     #这部分过滤了5300-1556=3744条
                        continue
                ...
                if len(input_seq) < args.short_traj_thres:     #这部分过滤了1556=14555-12999条
                    continue
    ...
    val_dataset = TrajectoryDatasetVal(val_df) # 12999   18299-12999=5300
