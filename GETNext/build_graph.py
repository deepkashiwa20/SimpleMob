...
if __name__ == '__main__':
    ...
    if args.dataset_name == 'nyc': 
        dataset_dir = f"../Foursquare-NYC"
    elif args.dataset_name == 'tky':
        dataset_dir = f"../Foursquare-TKY"
    else:
        dataset_dir = f"../Gowalla-CA"
    dst_dir = f"dataset/{args.dataset_name}"
    os.makedirs(dst_dir, exist_ok=True)

    # Build POI checkin trajectory graph
    train_df = pd.read_csv(os.path.join(dataset_dir, f"{args.dataset_name}_train.csv"))
